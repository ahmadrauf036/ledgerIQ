...
// ── Send invite ──────────────────────────────────
const sendInvite = async (body, invitedBy) => {
    ...
    const { data: existingInvite } = await supabase_1.supabaseAdmin
        .from("invites")
        .select("id, status")
        .eq("email", email)
...
const validateInvite = async (token) => {
    const { data: invite, error } = await supabase_1.supabaseAdmin
        .from("invites")
        .select(`
            *,
            companies (
...
const acceptInvite = async (body) => {
    ...
    const { invite, company_name } = await (0, exports.validateInvite)(token);
    // Create auth user
    const { data: authUser, error: createError } = await supabase_1.supabaseAdmin.auth.admin.createUser({
        email: invite.email,
        password,
        email_confirm: true,
        app_metadata: {
...
const getCompanyInvites = async (companyId) => {
