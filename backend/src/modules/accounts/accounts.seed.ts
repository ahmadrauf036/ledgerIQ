...
export const seedDefaultAccounts = async (companyId: string) => {
    ...
    for (const account of rootAccounts) {
        ...
        if (error)
            throw new Error(
                `Failed to seed account ${account.code}: ${error.message}`,
            );
    ...
    const childAccounts = DEFAULT_ACCOUNTS.filter(
        (a) => a.parent_code !== null,
    );
